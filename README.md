...
Four scripts exist under `scripts/` and run as-is — no setup beyond `cd` into the
repo root:

```bash
python scripts/build_scenarios.py
```
Rebuilds the 32-scenario library (`data/scenarios/scenarios.json`).

```bash
python scripts/check_scenarios.py
```
Runs every statically-evaluable scenario through the full L0–L5 engine and checks
each against its expected-behaviour envelope. Current output:
```
PASS 27 / FAIL 0  []
```
...
```
Builds a 5-record hash-chained audit log, verifies it, then tampers with record 2
and shows detection. Current output:
```
clean chain valid: True
after tampering record 2: False -> first broken index 2
```

```bash
python scripts/check_cohort.py
```
Generates a full 24-hour synthetic patient cohort for the tertiary (H-L) hospital
profile, admits every patient into a department, builds the Living Queue, and
verifies the audit chain over the whole run. Sample output:
```
{'n': 575, 'seed': 20260825, ... 'deteriorating_fraction': 0.057, ...}
K = 3 worklist = ['G-0565', 'G-0174', 'G-0540']
audit records: 575 chain valid: True
```

**There is no `scripts/run_demo`, no API server, and no UI.** The four scripts
above are the only way to exercise the system today.
...
- **No proper demo tooling.** `scripts/build_scenarios.py`,
  `scripts/check_scenarios.py`, `scripts/check_audit.py`, and
  `scripts/check_cohort.py` exist and run (§6), but there is still no
  `scripts/run_demo`, `scripts/generate_data`, `scripts/run_tests`,
  `scripts/run_evaluation`, or `scripts/verify_audit` — the check scripts are
  developer smoke tests, not a scripted, presentable demo.
