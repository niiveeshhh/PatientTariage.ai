...
```bash
python scripts/build_scenarios.py     # rebuild the 32-scenario library
python scripts/check_scenarios.py     # -> PASS 27 / FAIL 0  []
python scripts/check_audit.py         # -> tamper detected at first broken index 2
python scripts/check_cohort.py        # -> 575-patient cohort, K=3, chain valid
```

All four are real files under `scripts/`, not snippets — see README §6 for what
each one does and its current output. There is no test runner, no API, no UI, and
no scripted presentable demo yet, so there is still no `run_demo` command.
...
- **A scripted, presentable demo** — `scripts/run_demo`, `generate_data`,
  `run_tests`, `run_evaluation`, `verify_audit` do not exist. Four developer
  smoke-test scripts do exist and run cleanly (`build_scenarios.py`,
  `check_scenarios.py`, `check_audit.py`, `check_cohort.py` — see "How to run
  what exists" above), but none of them are a scripted walkthrough for a judge.
